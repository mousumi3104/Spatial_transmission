data {
  ...
  int<lower=1> W;
  matrix[final_time, M_regions] gm_non_res;
  matrix[final_time, M_regions] gm_res;
   ...
   for (m in 1:M_regions){
      for (t in (initial_seeding_day+1):final_time){
          Rt[t,m] = mu[m] * 2 * inv_logit(- weekly_effect[day_week_index[t], m]
                                     ...
                                     - (y1[m] * gm_non_res[t,m] * I[t,3]) //- (y2[m] * gm_res[t,m] * I[t,3])
                                     - (z1[m] * gm_non_res[t,m] * I[t,4]));// - (z2[m] * gm_res[t,m] * I[t,4]));
      }
     }
...
model {
 ...
 to_vector(weekly_effect_d) ~ normal(0, 1);
  ...
  for (t in fitting_death_start:death_data_length){
    //for (m in 1:M_regions){
      target += neg_binomial_2_lpmf(death[t,]| weekly_deaths[t,], phi1);
    //}
  }
}
