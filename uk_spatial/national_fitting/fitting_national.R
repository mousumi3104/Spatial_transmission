...
fitting_death_start <- which(cumsum(death_data) > 10)[1]     # week when cumulative death exceeds above 10
epidemic_start <- fitting_death_start - 6    # 30 days before the infection start (4 weeks and 1 week for the delay in data collection)
...
stan_data <- list(final_time=final_time,
                  ...
                  fitting_start = fitting_death_start)
                  # 
...
# m <- rstan::stan_model("fitting_national.stan")
...
#                       thin=1, 
#                       seed=12345)
m <- cmdstan_model("fitting_national.stan")

init_list <- list(list(Rt = 3.28, init_seed = 40),
                  list(Rt = 3.28, init_seed = 40),
                  list(Rt = 3.28, init_seed = 40),
                  list(Rt = 3.28, init_seed = 40))
...
fit <-  m$sample(
  ...
  adapt_delta = 0.9,
  max_treedepth = 10,
  init = init_list)     # adapt_delta controls acceptance probability (lower -> larger step size, higher acceptance rate, less time, less explored posterior distribution
...
estimated_daily_deaths <- fit$draws("daily_deaths", format="matrix")
plot(estimated_deaths_mean )
plot(colMeans(estimated_daily_deaths))


estimated_deaths <- fit$draws("weekly_deaths",format="matrix")
